...
def reset_user_password():
    email = input("Enter the user's email to reset password: ").strip()
    users = read_all_csv("Login.csv")
    for user in users:
        if user["Email"].lower() == email.lower():
            new_pwd = input("Enter new password: ")
            user["PasswordHash"] = hash_password(new_pwd)
            write_all_csv("Login.csv", users[0].keys(), users)
    ...
    for user in users:
        if user["Email"] == email:
            ...
            if user["PasswordHash"] == "":
                ...
                print("🔁 Please return to the main menu and login again.")
                return None, None, True  # third flag indicates first-time setup
            pwd = input("Enter password: ")
            if check_password(pwd, user["PasswordHash"]):
                print("✅ Login successful.")
                return email, expected_role, False
            else:
    ...
    while True:
        ...
        if role_choice == "0":
            ...
            if invalid_attempts == 3:
                print("⚠️ Too many invalid attempts. Exiting for safety.")
                exit()
            continue

        email, returned_role, password_just_set = login(expected_role=role)

        if returned_role is None:
            if password_just_set:
        ...
        # reset invalid_attempts on successful login
        invalid_attempts = 0
        ...
        if role == "Admin":
            while True:
                ...
                elif ch == "13": reset_user_password()
                elif ch in ["0", "back"]:
                    break
                else:
                    print("❌ Invalid choice")
